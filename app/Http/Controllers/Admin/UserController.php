...
class UserController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // load available roles for dropdowns - exclude admin
        $roles = Role::active()->where('slug', '!=', 'admin')->orderBy('name')->get();
    ...
    {
        // load roles so we can validate against them - exclude admin
        $availableRoleSlugs = Role::active()->where('slug', '!=', 'admin')->pluck('slug')->toArray();
    ...
    public function bulkApprove(Request $request)
    {
        ...
        foreach ($users as $user) {
            try {
                ...
                \Log::error('Bulk approval email dispatch failed for user ' . $user->id . ': ' . $e->getMessage());
                $failedEmails++;
            }
        }
        ...
        if ($failedEmails > 0) {
            $message .= " (Note: {$failedEmails} approval email(s) failed to send.)";
        }
    ...
    public function approve(Request $request, $id)
    {
        ...
        try {
            ...
            \Log::error('Approval email dispatch failed for user ' . $user->id . ': ' . $e->getMessage());
        }
