...
class InventoryController extends Controller
{
    ...
    public function index()
    {
        ...
        $lowStockProducts = Product::with('stock')
            ->whereHas('stock', fn($q) => $q->whereRaw('quantity <= min_quantity'))
            ->take(10)
            ->get();
    ...
    public function processStockOut(Request $request)
    {
        ...
        try {
            ...
            if ($currentStock < $request->input('quantity')) {
                return response()->json([
                    'success' => false,
                    'message' => 'Insufficient stock available. Current stock: ' . $currentStock
                ], 400);
            }
    ...
    public function history(Request $request, $id)
    {
        try {
            ...
            $query = StockTransaction::with('user')->where('product_id', $id);
