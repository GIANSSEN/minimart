...
class ProductController extends Controller
{
    ...
    public function destroy(Product $product)
    {
        try {
            ...
            // Check for various dependencies that prevent deletion
            $hasSaleItems = DB::table('sale_items')->where('product_id', $product->id)->exists();
            $hasSalesReturns = DB::table('sales_returns')->where('product_id', $product->id)->exists();
            $hasSupplierReturns = DB::table('supplier_returns')->where('product_id', $product->id)->exists();
            $hasStockTransactions = $product->stockTransactions()->count() > 0;

            if ($hasSaleItems || $hasSalesReturns || $hasSupplierReturns || $hasStockTransactions) {
                $dependency = $hasSaleItems ? 'sales records' :
                    ($hasSalesReturns ? 'sales returns' :
                        ($hasSupplierReturns ? 'supplier returns' : 'transaction history'));

                if (request()->ajax() || request()->wantsJson()) {
                    return response()->json([
                        'success' => false,
                        'message' => "Cannot delete product that has {$dependency}. Try deactivating it instead."
                    ], 422);
                }
                return back()->with('error', "Cannot delete product that has {$dependency}.");
            }
            ...
            DB::rollBack();
            $message = 'Error deleting product. It may be referenced by other records.';
            if ($e->getCode() == "23000") {
                $message = 'Cannot delete this product because it is referenced in other modules (e.g., Sales, Inventory).';
            }
        ...
        try {
            ...
            foreach ($request->input('ids') as $id) {
                ...
                if ($product) {
                    ...
                    $hasDeps = DB::table('sale_items')->where('product_id', $id)->exists() ||
                        DB::table('sales_returns')->where('product_id', $id)->exists() ||
                        DB::table('supplier_returns')->where('product_id', $id)->exists() ||
                        $product->stockTransactions()->count() > 0;

                    if ($hasDeps) {
                        $skippedCount++;
                        continue; // Skip products with dependencies
                    }
            ...
            ActivityLog::create([
                ...
                'action_type' => 'DELETE',
                'description' => "Bulk deleted {$deletedCount} products" . ($skippedCount > 0 ? " ({$skippedCount} skipped)" : ""),
                'ip_address' => $request->ip()
            ]);
            ...
            if ($skippedCount > 0) {
                $message .= " " . $skippedCount . " were skipped because they have transaction history.";
            }
