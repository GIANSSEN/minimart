    ...
    (function() {
        ...
        if (filterForm) {
            const debouncedFetch = debounce(() => {
                ...
                const url = `{{ route('admin.products.index') }}?${params}&t=${Date.now()}`;
                fetchProducts(url);
        ...
        bulkDeleteBtn?.addEventListener('click', async function() {
            ...
            if (result.isConfirmed) {
                ...
                try {
                    ...
                    if (data.success) {
                        // Only remove what was actually deleted on the server
                        ...
                        Swal.fire({
                            icon: data.skipped_count > 0 ? 'info' : 'success',
                            title: data.skipped_count > 0 ? 'Partially Completed' : 'Deleted!',
                            text: data.message,
                            timer: data.skipped_count > 0 ? 5000 : 2000,
                            showConfirmButton: data.skipped_count > 0
                        });
                        updateBulkActions();
                        fetchProducts(window.location.href + '&t=' + Date.now());
                    }
                } catch (error) {
                    Swal.fire({
                        icon: 'error',
                        title: 'Error',
                        text: error.message || 'Could not delete products.'
                    });
                }
            }
        });
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        Swal.fire({
                            ...
                            fetchProducts(url + separator + 't=' + Date.now()); 
                        });
                    }
                } catch (error) {
                    Swal.fire({
        ...
        window.showProduct = async function(id) {
            ...
            try {
                const res = await fetch(`{{ url('admin/products') }}/${id}`, {
                ...
                const productImagePlaceholder = `<div style="width:100%;height:100%;background:#f8fafc;display:flex;align-items:center;justify-content:center;border-radius:12px;border:1px dashed #e2e8f0;"><span style="color:#94a3b8;font-size:0.8rem;font-weight:600;">NO IMAGE</span></div>`;
                const imgHtml = p.image
                    ? `<img src="${normalizeImagePath(p.image)}" alt="${p.product_name}" style="width:100%;height:100%;object-fit:contain;border-radius:12px;" onerror="this.parentElement.innerHTML='${productImagePlaceholder.replace(/"/g, '&quot;').replace(/'/g, '\\\'')}';">`
        ...
        window.submitEditForm = async function() {
            ...
            try {
                ...
                const res = await fetch(`{{ url('admin/products') }}/${editingProductId}`, {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': csrfToken,
                        'X-Requested-With': 'XMLHttpRequest',
