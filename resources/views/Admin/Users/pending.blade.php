...
.sticky-quick-info {
    position: sticky;
    top: 0;
    z-index: 1020; /* Above regular content but below some modals */
    background: #f8fafc;
    padding: 1rem 0;
    margin: -1rem 0 1rem 0;
    box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05); /* Subtle shadow when sticky */
}
...
window.showUser = async function(id) {
    const modalElement = document.getElementById('userShowModal');
                ...
                <div class="row g-4">
                    <div class="col-md-4 text-center border-end">
                        <div class="position-relative d-inline-block mb-3">
                            <img src="${user.avatar_url || 'https://ui-avatars.com/api/?name='+encodeURIComponent(user.full_name)+'&background=6366f1&color=fff&size=100'}" class="rounded-circle shadow-sm border border-4 border-white" style="width: 100px; height: 100px; object-fit: cover;">
                            <span class="position-absolute bottom-0 end-0 translate-middle p-2 bg-warning border border-light rounded-circle" title="Pending Approval"></span>
                        </div>
                        <h5 class="fw-bold mb-1">${user.full_name}</h5>
                        <p class="text-muted small mb-3">${user.email}</p>
                        <span class="badge bg-primary-soft text-primary px-3 py-2 rounded-pill text-uppercase fw-bold" style="font-size: 0.7rem;">${user.role}</span>
                    </div>
                    <div class="col-md-8">
                        <div class="row g-3">
                            <div class="col-6">
                                <div class="p-3 bg-light rounded-3">
                                    <label class="small fw-bold text-muted text-uppercase d-block mb-1">Employee ID</label>
                                    <p class="mb-0 text-dark fw-bold">#${user.employee_id || '—'}</p>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="p-3 bg-light rounded-3">
                                    <label class="small fw-bold text-muted text-uppercase d-block mb-1">Username</label>
                                    <p class="mb-0 text-dark fw-bold">${user.username || '—'}</p>
                                </div>
                            </div>
                            <div class="col-6">
                                <label class="small fw-bold text-muted text-uppercase d-block mb-1 px-1">Phone Number</label>
                                <p class="mb-0 text-dark fw-medium px-1"><i class="fas fa-phone-alt me-2 text-muted"></i>${user.phone || '—'}</p>
                            </div>
                            <div class="col-6">
                                <label class="small fw-bold text-muted text-uppercase d-block mb-1 px-1">Registered On</label>
                                <p class="mb-0 text-dark fw-medium px-1"><i class="fas fa-calendar-alt me-2 text-muted"></i>${new Date(user.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</p>
                            </div>
                            <div class="col-12">
                                <div class="mt-2 p-3 border rounded-3 bg-white">
                                    <label class="small fw-bold text-muted text-uppercase d-block mb-1">Home Address</label>
                                    <p class="mb-0 text-dark small"><i class="fas fa-map-marker-alt me-2 text-muted"></i>${user.address || 'No address provided'}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <tr class="approval-row" id="user-row-{{ $user->id }}">
                        ...
                        <td data-label="Actions" class="text-end">
                            <div class="actions-flex">
                                ...
                                <button type="button" onclick="showUser({{ $user->id }})" class="btn-action-view" title="View Profile">
...
{{-- USER SHOW MODAL --}}
<div class="modal fade modal-premium" id="userShowModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 20px;">
            <div class="modal-header border-0 p-4" style="background: linear-gradient(135deg, #1E293B 0%, #334155 100%); color: white; border-radius: 20px 20px 0 0;">
                <h5 class="modal-title fw-bold">User Details</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-0" id="userShowContent">
