                    ...
                    <tr data-brand-id="{{ $brand->id }}">
                        ...
                        <td data-label="Actions">
                            <div class="d-flex justify-content-center gap-2">
                                <button type="button" 
                                        class="btn-view" 
                                        onclick="viewBrandDetails('{{ $brand->id }}')" 
                                        title="View Details">
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 20px; overflow: hidden;">
            <div class="modal-header bg-primary text-white py-3">
                <h5 class="modal-title fw-bold" id="viewBrandModalLabel">
                    <i class="fas fa-trademark me-2"></i>Brand Information
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-0" id="brandModalBody">
                <div class="text-center py-5" id="brandLoading">
                    <div class="spinner-border text-primary" role="status">
                        <span class="visually-hidden">Loading...</span>
                    </div>
                    <p class="mt-2 text-muted">Retrieving brand details...</p>
                </div>
                <div id="brandContent" style="display: none;">
                    <div class="brand-hero p-4 text-center" style="background: #f8fafc; border-bottom: 1px solid #e2e8f0;">
                        <div id="brandLogoContainer" class="mb-3">
                            <!-- Logo will be inserted here -->
                        </div>
                        <h3 id="modalBrandName" class="fw-bold mb-1"></h3>
                        <p id="modalBrandCode" class="badge bg-light text-primary border px-3 py-2 fs-6"></p>
                    </div>
                    ...
                    <div class="p-4">
                        <div class="row g-4">
                            <div class="col-md-7">
                                ...
                                <p id="modalBrandDescription" class="text-secondary" style="line-height: 1.6;"></p>
                                
                                <div class="mt-4">
                                    <h6 class="fw-bold text-uppercase small text-muted mb-3 border-bottom pb-2">Website</h6>
                                    <div id="modalBrandWebsiteContainer">
                            ...
                            <div class="col-md-5">
                                <div class="card bg-light border-0 rounded-4 p-4 text-center h-100">
                                    <h6 class="fw-bold text-uppercase small text-muted mb-2">Inventory Stats</h6>
                                    <div class="display-4 fw-bold text-primary mb-1" id="modalProductCount">0</div>
                                    ...
                                    <div class="d-flex justify-content-between align-items-center small px-2">
                                        <span class="text-muted">Total Stocks:</span>
                                        <span class="fw-bold" id="modalTotalStocks">0 units</span>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="modal-footer bg-light border-0 py-3">
                ...
                <a id="modalEditLink" href="#" class="btn btn-primary px-4 rounded-pill">
...
function viewBrandDetails(brandId) {
    const modalElement = document.getElementById('viewBrandModal');
    ...
    const websiteContainer = document.getElementById('modalBrandWebsiteContainer');
    ...
    // Build the edit URL
    const editUrlTemplate = "{{ route('admin.brands.edit', ':id') }}";
    editLink.href = editUrlTemplate.replace(':id', brandId);
    ...
    fetch(`{{ url('admin/brands') }}/${brandId}`, {
        ...
        .then(data => {
            if (data.brand) {
                ...
                // Basic info
                document.getElementById('modalBrandName').textContent = brand.brand_name;
                document.getElementById('modalBrandCode').textContent = brand.brand_code || 'No Brand Code';
                document.getElementById('modalBrandDescription').textContent = brand.description || 'No description available for this brand.';
                document.getElementById('modalProductCount').textContent = data.product_count || 0;
                document.getElementById('modalTotalStocks').textContent = (data.total_stocks || 0) + ' units';
                
                // Logo
                logoContainer.innerHTML = '';
                if (brand.logo) {
                    logoContainer.innerHTML = `<img src="{{ asset('') }}${brand.logo}" alt="${brand.brand_name}" class="img-fluid rounded" style="max-height: 80px; object-fit: contain;">`;
                } else {
                    logoContainer.innerHTML = `<div class="bg-primary text-white d-inline-flex align-items-center justify-content-center rounded-circle" style="width: 80px; height: 80px; font-size: 2rem; font-weight: 700;">${brand.brand_name.substring(0, 2).toUpperCase()}</div>`;
                }
                
                // Website
                websiteContainer.innerHTML = '';
                if (brand.website) {
                    websiteContainer.innerHTML = `<a href="${brand.website}" target="_blank" class="text-primary text-decoration-none fw-bold d-flex align-items-center justify-content-center justify-content-md-start">
                        <i class="fas fa-external-link-alt me-2"></i>${brand.website}
                    </a>`;
                } else {
                    websiteContainer.innerHTML = `<span class="text-muted italic">No website registered</span>`;
                }
                ...
                Swal.fire('Error', 'Brand details not found', 'error');
                modal.hide();
            }
        })
        .catch(error => {
            console.error('Error:', error);
            Swal.fire('Error', 'Failed to fetch brand details. Please try again.', 'error');
            modal.hide();
        });
}
