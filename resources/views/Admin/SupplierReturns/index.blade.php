...
<div class="returns-page">
    {{-- Premium Header --}}
    <div class="page-header-premium d-flex justify-content-between align-items-center mb-4 bg-white p-4 rounded-4 shadow-sm">
        <div class="header-left d-flex align-items-center">
            <div class="header-icon-box p-3 rounded-4 me-3" style="background: linear-gradient(135deg, #ef4444 0%, #b91c1c 100%); color: white;">
                <i class="fas fa-undo fa-lg"></i>
            </div>
            <div class="header-text">
                <h1 class="page-title h3 fw-bold mb-0">Supplier Returns</h1>
                <p class="page-subtitle text-muted mb-0">Manage damaged or expired inventory returns</p>
            </div>
        </div>
        <div class="header-actions">
            <button onclick="addReturn()" class="btn btn-danger rounded-3 px-4 py-2 fw-bold">
    ...
    {{-- Filter Card --}}
    <div class="filter-card-premium mt-4">
    ...
    {{-- Table Content --}}
    <div class="content-card-premium mt-4 bg-white rounded-4 shadow-sm overflow-hidden">
                ...
                <tbody>
                    @if(count($returns) > 0)
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge bg-light text-dark border p-2">{{ $ret->supplier->supplier_name ?? 'N/A' }}</span>
                            </td>
                            <td>
                        ...
                        <tr>
                            <td colspan="7" class="text-center py-5">
                                <div class="opacity-30">
                                    <i class="fas fa-undo fa-4x mb-3 text-muted"></i>
                                    <h3 class="fw-bold text-muted">No Records</h3>
                                    <p class="text-muted">No supplier returns have been logged yet</p>
                                </div>
                            </td>
                        </tr>
                    @endif
...
async function addReturn() {
    Swal.fire({
            ...
            <div class="text-start p-2">
                <div class="row g-3">
                    <div class="col-12">
                        ...
                        <select id="swal_supplier" class="form-select rounded-3 shadow-sm border-0 bg-light">
                    ...
                    <div class="col-12">
                        ...
                        <select id="swal_product" class="form-select rounded-3 shadow-sm border-0 bg-light">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" id="swal_qty" class="form-control rounded-3 shadow-sm border-0 bg-light" value="1" min="1">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label small fw-bold text-muted mb-1">RETURN DATE</label>
                        <input type="date" id="swal_date" class="form-control rounded-3 shadow-sm border-0 bg-light" value="{{ date('Y-m-d') }}">
                    </div>
                    <div class="col-12">
                        <label class="form-label small fw-bold text-muted mb-1">PRIMARY REASON</label>
                        <textarea id="swal_reason" class="form-control rounded-3 shadow-sm border-0 bg-light" rows="3" placeholder="Explain why this item is being returned..."></textarea>
                    </div>
                    <div class="col-12">
                        <label class="form-label small fw-bold text-muted mb-1">STATUS</label>
                        <select id="swal_status" class="form-select rounded-3 shadow-sm border-0 bg-light">
        ...
        confirmButtonText: '<i class="fas fa-plus-circle me-2"></i>Record Return',
...
async function editReturn(id) {
    ...
    try {
        const res = await fetch(`{{ url('admin/supplier-returns') }}/${id}`, { headers: { 'X-Requested-With': 'XMLHttpRequest' } });
        ...
        Swal.fire({
                        ...
                        <div class="col-12">
                            ...
                            <select id="swal_supplier" class="form-select rounded-3 shadow-sm border-0 bg-light">
                                @foreach ($suppliers as $s) 
                                    <option value="{{ $s->id }}" ${d.supplier_id == {{ $s->id }} ? 'selected' : ''}>{{ $s->supplier_name }}</option> 
                                @endforeach
                            </select>
                        </div>
                        <div class="col-12">
                            ...
                            <select id="swal_product" class="form-select rounded-3 shadow-sm border-0 bg-light">
                                @foreach ($products as $p) 
                                    <option value="{{ $p->id }}" ${d.product_id == {{ $p->id }} ? 'selected' : ''}>{{ $p->product_name }}</option> 
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label small fw-bold text-muted mb-1">QUANTITY</label>
                            <input type="number" id="swal_qty" class="form-control rounded-3 shadow-sm border-0 bg-light" value="${d.quantity}" min="1">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label small fw-bold text-muted mb-1">RETURN DATE</label>
                            <input type="date" id="swal_date" class="form-control rounded-3 shadow-sm border-0 bg-light" value="${d.return_date}">
                        </div>
                        <div class="col-12">
                            <label class="form-label small fw-bold text-muted mb-1">PRIMARY REASON</label>
                            <textarea id="swal_reason" class="form-control rounded-3 shadow-sm border-0 bg-light" rows="3" placeholder="Explain why this item is being returned...">${d.reason}</textarea>
                        </div>
                        <div class="col-12">
                            <label class="form-label small fw-bold text-muted mb-1">STATUS</label>
                            <select id="swal_status" class="form-select rounded-3 shadow-sm border-0 bg-light">
            ...
            `,
            showCancelButton: true,
            confirmButtonText: '<i class="fas fa-save me-2"></i>Update Entry',
...
async function saveReturn(id, data) {
    ...
    const url = id ? `{{ url('admin/supplier-returns') }}/${id}` : '{{ route("admin.supplier-returns.store") }}';
    try {
...
function deleteReturn(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                const res = await fetch(`{{ url('admin/supplier-returns') }}/${id}`, {
                    method: 'DELETE',
