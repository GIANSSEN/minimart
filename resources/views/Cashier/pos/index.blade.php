                ...
                <div class="row align-items-center">
                    <div class="col-lg-9 d-flex align-items-center gap-3">
                        ...
                        <div class="welcome-text-compact">
                            ...
                            <p class="mb-0 small opacity-90 text-white">
                                <i class="fas fa-user-circle me-1 text-white opacity-75"></i>
                                Cashier: <strong class="text-white">{{ Auth::user()->full_name }}</strong>
                                <span class="ms-3 border-start border-white-50 ps-3">
                                    <i class="far fa-calendar-alt me-1 text-white opacity-75"></i>
                                    {{ now()->format('l, F j, Y') }}
                                </span>
                                <span class="ms-3 border-start border-white-50 ps-3">
                                    <i class="far fa-clock me-1 text-white opacity-75"></i>
                                    <span id="realTimeClock">{{ now()->format('h:i:s A') }}</span>
                                </span>
                            </p>
                        </div>
                    </div>
                ...
                <div class="card-body">
                    ...
                    <div class="cart-summary p-4 bg-light border-top" id="cartSummary" style="display: none;">
                        ...
                        <div class="d-flex justify-content-between align-items-center pt-3 border-top mt-2">
                            ...
                            <h2 class="mb-0 fw-900" style="color: var(--pos-orange); font-size: 2.2rem;" id="total">₱0.00</h2>
                        </div>
                    </div>
...
<!-- ============================================================ -->
<!-- Barcode Scanner Modal – fully functional html5-qrcode       -->
<!-- ============================================================ -->
        ...
        <div class="modal-content border-0 shadow-lg" style="border-radius: 20px; overflow: hidden;">
            ...
            <div class="modal-header text-white py-3 px-4"
                ...
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div>
                        <h5 class="modal-title fw-bold mb-0" id="scannerModalLabel">Barcode Scanner</h5>
                        <p class="small mb-0 opacity-75">Point your camera at any barcode</p>
                    </div>
                </div>
            ...
            <div class="modal-body p-4">
                ...
                <!-- Camera Viewport -->
                <div class="scanner-viewport mb-3 position-relative rounded-3 overflow-hidden"
                     style="background:#0f172a; min-height:240px;">
                    <!-- html5-qrcode renders into this div -->
                    <div id="reader" style="width:100%;"></div>

                    <!-- Scan-line animation overlay (shows while scanning) -->
                    <div id="scannerOverlay" class="position-absolute top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center" style="pointer-events:none;">
                        <div class="scan-frame position-relative">
                    ...
                    <!-- Paused state label -->
                    <div id="scannerPaused" class="position-absolute top-50 start-50 translate-middle text-center d-none">
                        <i class="fas fa-pause-circle text-white opacity-50" style="font-size:3rem;"></i>
                        <p class="text-white small opacity-75 mt-2 mb-0">Scanner paused</p>
                    </div>
                </div>

                <!-- Status bar -->
                <div id="scannerStatus" class="alert alert-info py-2 px-3 small mb-3 d-flex align-items-center gap-2">
                    <div class="spinner-border spinner-border-sm text-primary flex-shrink-0" role="status" style="width:14px;height:14px;"></div>
                    <span>Initializing camera…</span>
                </div>

                <!-- ── Product preview (hidden until scan succeeds) ── -->
                <div id="scannedProductCard" class="d-none mb-3 p-3 rounded-3 border"
                    ...
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div class="flex-grow-1 min-width-0">
                            <div class="fw-bold text-dark mb-0" id="scannedProductName" style="font-size:.9rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"></div>
                            <div class="small text-muted">
                                Price: <strong class="text-success" id="scannedProductPrice"></strong>
                                &nbsp;|&nbsp; Stock: <strong id="scannedProductStock"></strong>
                            </div>
                        </div>
                        <button class="btn btn-success btn-sm rounded-3 fw-bold flex-shrink-0" id="addScannedBtn" onclick="confirmAddScanned()">
                ...
                <div>
                    <label class="form-label small fw-semibold text-muted text-uppercase mb-2" style="letter-spacing:.5px;">
                        <i class="fas fa-keyboard me-1"></i>Manual Entry
                    </label>
                    <div class="input-group">
                        <input type="text" id="manualBarcode" class="form-control rounded-start-3" 
                                   placeholder="Type or paste barcode..."
                                   onkeydown="if(event.key==='Enter') processManualBarcode()">
                        <button class="btn btn-primary rounded-end-3 px-3 fw-semibold" onclick="processManualBarcode()">
            ...
            <div class="modal-footer border-0 px-4 pb-4 pt-0 d-flex justify-content-between">
                <div id="scannerToggleBtns" class="d-flex gap-2">
                    <button class="btn btn-outline-danger btn-sm rounded-3" id="stopScannerBtn" onclick="stopScanner()" style="display:none!important;">
                        <i class="fas fa-stop-circle me-1"></i>Stop Camera
                    </button>
                    <button class="btn btn-outline-primary btn-sm rounded-3" id="resumeScannerBtn" onclick="resumeScanner()" style="display:none!important;">
...
<style>
    ...
    .welcome-card-compact {
        ...
        padding: 15px 25px;
        color: white;
        border-left: 5px solid var(--pos-orange);
        position: relative;
        overflow: hidden;
    }
    ...
    :root {
        ...
        --pos-orange: #f97316;
        --pos-orange-light: #fb923c;
        --pos-orange-dark: #ea580c;
        --pos-orange-soft: #fff7ed;
    }

    .bg-dark-blue {
        background: linear-gradient(135deg, var(--pos-blue-dark) 0%, var(--pos-blue) 100%) !important;
    }
    ...
    .btn-primary:hover, .btn-primary:focus {
        background-color: var(--pos-blue-dark);
        border-color: var(--pos-blue-dark);
        color: white;
    }
    ...
    .scan-btn {
        border-radius: 12px;
        padding: 0 20px;
        background: linear-gradient(135deg, var(--pos-orange) 0%, var(--pos-orange-dark) 100%);
        border: none;
        color: white;
    }
    ...
    .change-box-premium {
        ...
        border-right: 6px solid var(--pos-orange);
    ...
    .view-btn:hover { color: var(--pos-blue); }
    .view-btn.active {
    ...
    .customer-type-card:hover i { color: var(--pos-blue); }
    .btn-check:checked + .customer-type-card {
        border-color: var(--pos-blue);
        background: linear-gradient(135deg, #dbeafe, var(--pos-blue-soft));
        color: var(--pos-blue-dark);
        box-shadow: 0 2px 10px rgba(37, 99, 235, 0.25);
    }
    .btn-check:checked + .customer-type-card i { color: var(--pos-blue-dark); }
    ...
    .payment-method-card:hover i { color: var(--pos-blue); }
    ...
    .btn-check:checked + .payment-method-card i { color: var(--pos-blue-dark); }

    /* ==================== ADD TO CART BUTTON ==================== */
    .btn-add-cart {
        background: linear-gradient(135deg, var(--pos-blue), var(--pos-blue-dark));
        color: #fff;
        border: none;
        border-radius: 8px;
        font-weight: 600;
    ...
    .btn-add-cart:hover {
        background: linear-gradient(135deg, var(--pos-blue-dark), var(--pos-blue));
        color: #fff;
    ...
    .scan-corner.tl { top: -2px; left: -2px; border-top-width: 5px; border-left-width: 5px; border-top-left-radius: 15px; }
    .scan-corner.tr { top: -2px; right: -2px; border-top-width: 5px; border-right-width: 5px; border-top-right-radius: 15px; }
    .scan-corner.bl { bottom: -2px; left: -2px; border-bottom-width: 5px; border-left-width: 5px; border-bottom-left-radius: 15px; }
    .scan-corner.br { bottom: -2px; right: -2px; border-bottom-width: 5px; border-right-width: 5px; border-bottom-right-radius: 15px; }
...
<script>
    ...
    function startBarcodeScanner() {
        ...
        statusEl.innerHTML = `
            ...
            <span>Initializing camera…</span>
        `;
        statusEl.className = "alert alert-info py-2 px-3 small mb-3 d-flex align-items-center gap-2";
        document.getElementById('scannerOverlay').style.display = 'flex';
        document.getElementById('scannerPaused').classList.add('d-none');
        document.getElementById('scannedProductCard').classList.add('d-none');
        document.getElementById('resumeScannerBtn').setAttribute('style', 'display:none!important');
    ...
    function initCamera() {
        ...
        html5QrCode.start({ facingMode: "environment" }, config, successCallback)
            .then(() => updateScannerReady())
            .catch(err => {
                console.warn("Env camera failed, trying default", err);
                // Fallback to any available camera
                html5QrCode.start({ facingMode: "user" }, config, successCallback)
                    .then(() => updateScannerReady())
                    .catch(e => handleScannerError(e));
            });
    }

    function updateScannerReady() {
        const statusEl = document.getElementById('scannerStatus');
        statusEl.innerHTML = `<i class="fas fa-check-circle text-success fs-6"></i> <span>Scanner Ready - Point at a barcode</span>`;
        statusEl.className = "alert alert-success py-2 px-3 small mb-3 d-flex align-items-center gap-2";
        scannerActive = true;
    }
    ...
    function handleScannerError(err) {
        ...
        statusEl.innerHTML = `
            <div class="d-flex align-items-center justify-content-between w-100">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <span>Camera Error: ${err.name || 'Failed'}</span>
                </div>
                <button class="btn btn-sm btn-outline-danger py-0 px-2" onclick="initCamera()" style="font-size:0.75rem">Retry</button>
            </div>`;
        statusEl.className = "alert alert-danger py-2 px-3 small mb-3 d-flex align-items-center gap-2";
        ...
        document.getElementById('reader').innerHTML = `
            <div class="d-flex flex-column align-items-center justify-content-center h-100 text-center p-4">
                ...
                <p class="text-muted small mb-3">Could not start camera. Please ensure permissions are granted and no other app is using it.</p>
                <button class="btn btn-primary btn-sm rounded-pill px-4" onclick="initCamera()">
    ...
    function processScannedBarcode(barcode) {
        ...
        if (item) {
            addToCartById(item.dataset.id);
            showInstantFeedback(item.dataset.name);
        } else {
            // Search via API if not in local list
            fetch(`{{ url('cashier/pos/product') }}/${barcode}`)
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        addToCart(tempEl);
                        showInstantFeedback(data.product.name);
                    } else {
                        statusEl.innerHTML = `<i class="fas fa-times-circle text-danger fs-6"></i> <span>Not Found: ${barcode}</span>`;
                        statusEl.className = "alert alert-danger py-2 px-3 small mb-3 d-flex align-items-center gap-2";
                        // Resume logic if we were pausing, but we are not pausing now for speed
                    }
                });
        }
    }

    function showInstantFeedback(name) {
        // Visual & Audio Feedback
        scanSound.play().catch(e => {});
        document.getElementById('scannerOverlay').classList.add('scan-success-flash');
        
        const statusEl = document.getElementById('scannerStatus');
        const oldHtml = statusEl.innerHTML;
        const oldClass = statusEl.className;
        
        statusEl.innerHTML = `<i class="fas fa-cart-plus text-success fs-6"></i> <span>Added: ${name}</span>`;
        statusEl.className = "alert alert-success py-2 px-3 small mb-3 d-flex align-items-center gap-2 shadow-sm animate__animated animate__pulse";
        
        setTimeout(() => {
            document.getElementById('scannerOverlay').classList.remove('scan-success-flash');
    ...
    function showScannedPreview(id, name, price, stock) {
        // Pause scanner
        if (html5QrCode && scannerActive) {
            html5QrCode.pause();
            document.getElementById('scannerOverlay').style.display = 'none';
            document.getElementById('scannerPaused').classList.remove('d-none');
        }

        currentlyScannedItem = { id, name, price, stock };
        
        // Show preview card
        document.getElementById('scannedProductName').textContent = name;
        document.getElementById('scannedProductPrice').textContent = `₱${parseFloat(price).toFixed(2)}`;
        document.getElementById('scannedProductStock').textContent = stock;
        document.getElementById('scannedProductCard').classList.remove('d-none');
        
        // Update status
        const statusEl = document.getElementById('scannerStatus');
        statusEl.innerHTML = `<i class="fas fa-search-plus text-primary fs-6"></i> <span>Product Found! Confirm to add.</span>`;
        statusEl.className = "alert alert-primary py-2 px-3 small mb-3 d-flex align-items-center gap-2";
        
        // Show control buttons
        document.getElementById('resumeScannerBtn').setAttribute('style', 'display:inline-block!important');
    }

    function confirmAddScanned() {
        if (!currentlyScannedItem) return;
        
        addToCartById(currentlyScannedItem.id);
        
        // Feedback
        const addBtn = document.getElementById('addScannedBtn');
        const oldHtml = addBtn.innerHTML;
        addBtn.innerHTML = '<i class="fas fa-check"></i> Added';
        addBtn.className = "btn btn-success btn-sm rounded-3 fw-bold flex-shrink-0 disabled";
        
        setTimeout(() => {
            addBtn.innerHTML = oldHtml;
            addBtn.className = "btn btn-success btn-sm rounded-3 fw-bold flex-shrink-0";
            resumeScanner();
        }, 800);
    }
    ...
    function processManualBarcode() {
        const barcodeEl = document.getElementById('manualBarcode');
        const barcode = barcodeEl.value.trim();
        if (barcode) {
            processScannedBarcode(barcode);
            barcodeEl.value = ''; // Clear for next use
            // Focus back after a short delay
    ...
    document.getElementById('searchProduct').addEventListener('input', function() {
        ...
        if (search.length >= 4) { // Typical minimum barcode length
            const exactItem = Array.from(document.querySelectorAll('.product-item')).find(item => 
                (item.dataset.barcode || '').toLowerCase() === search
            );
            
            if (exactItem) {
                addToCartById(exactItem.dataset.id);
